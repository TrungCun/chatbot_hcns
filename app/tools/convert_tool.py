import os
os.environ["CUDA_VISIBLE_DEVICES"] = "0"  # must be set before torch import
...
class FileConverter:
    ...
    Được thiết kế để services và API gọi đến dùng.
    ...
    def process_folder(self, input_dir: str, output_dir: str | None = None) -> dict:
        """
        Chuyển đổi toàn bộ file hợp lệ trong thư mục (đệ quy).
        Returns: summary dict với keys: timestamp, total, success, failed.
        """
        ...
        for file_path in files:
            if Path(file_path).with_suffix(".md").exists():
                logger.info("Skipping (already converted): %s", Path(file_path).name)
                continue
            logger.info("Processing: %s", Path(file_path).name)
            result = self.convert_file(file_path, output_dir)
            if result["success"]:
                summary["success"].append(result["file"])
            else:
                summary["failed"].append({"file": result["file"], "error": result["error"]})

        summary_path = Path(output_dir) / "summary.json"
        summary_path.write_text(json.dumps(summary, indent=2, ensure_ascii=False), encoding="utf-8")
        logger.info("Done. Success: %d / %d", len(summary["success"]), len(files))
        return summary
...
def main():
    ...
    folders = ["regulation", "procedures", "forms", "departments", "guides"]
    base_dir = "/home/trung-ai/chatbot_hcns/pdf"

    with FileConverter(model_id) as converter:
        for folder in folders:
            logger.info("--- Processing folder: %s ---", folder)
            converter.process_folder(os.path.join(base_dir, folder))
